...
| Version | Notes |
|---------|-------|
| 1.9.1 | **Board filter redesign:** Category chips replaced with fixed tier chips (Recruit → Legend). Each chip is color-coded to its rank accent. Selecting a tier clears any active category filter. Board defaults to Recruit on open. Contractor NPC clothing list no longer duplicates on each reload (`ObjectCreationHandling.Replace`). |
